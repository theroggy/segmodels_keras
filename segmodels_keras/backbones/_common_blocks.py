"""Common blocks for backbone models.
...
def slice_tensor(x, start, stop, axis):
    if axis == 3:
        ...
        raise ValueError(f"Slice axis should be in (1, 3), got {axis}.")
...
def GroupConv2D(
    ...
    **kwargs,  # noqa: ARG001
    ...
    """Grouped Convolution Layer implemented as a Slice, Conv2D and Concatenate layers.

    Split filters to groups, apply Conv2D and concatenate back.
    ...
    Output shape:
        4D tensor with shape: (batch, new_rows, new_cols, filters) if data_format is
...
def expand_dims(x, channels_axis):
    if channels_axis == 3:
        ...
        raise ValueError(f"Slice axis should be in (1, 3), got {channels_axis}.")
